//each light can have brightness of 0 or more 
//LIGHTS START AT ZERO
// turn on =  increase by 1 (+1)
//turn off = decrease by 1 (-1) to a min of 0
//toggle = increase by 2 (+2)
...
for (let x = 0; x < gridSize; x++) {
    ...
    for (let y = 0; y < gridSize; y++) {
        row.push(0); // starts at 0 so this was changed
    }
    lights.push(row);
}
...
for (const line of lines) {
    ...
    for (let x = x1; x <= x2; x++) {
        for (let y = y1; y <= y2; y++) {
            if (action === "on") {
                lights[x][y] += 1; // turn on
            } else if (action === "off") {
                if (lights[x][y] >= 1) lights[x][y] -= 1; // turn off
...
// checking levels of lights 
//loop over every row (x), then over every column (y) to check
//the brightness and add to the count

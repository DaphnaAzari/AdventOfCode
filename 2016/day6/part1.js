...
for (let col = 0; col < columns.length; col++) {
    const mostUsed = FindMostUsedLetter(columns[col]);
...
function FindMostUsedLetter(letters) {
    //letters seen:
    const seenLetters = [];
    //count of letters:
    ...
    //now I need to find the max used
    //maxIndex starts at 0,  assuming the first letter is the most common
    let maxIndex = 0;

    countOfLetters.forEach((count, i) => {
        //countOfLetters[maxIndex] is the count of the current most common letter
        // I need to compare the current letter count which is count
        // with the current max.
        if (count > countOfLetters[maxIndex]) {
